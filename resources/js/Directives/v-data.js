            ...
            data.forEach(item => {
                ...
                columns.forEach(column => {
                    ...
                    if(fieldPath[0] === 'action'){
                        td.innerHTML = `<a data-id="${item.ID || item.id}" class="edit cursor-pointer"><i class="fas fa-edit mr-2"></i>Sửa</a>`;
                        tr.appendChild(td);
                    } else{
                        if(column.enums){
                            ...
                            }else if(column.field.includes("gia") || column.field.includes("tong")){
                                td.textContent = formatMoney(parseInt(value));
                            }else{
